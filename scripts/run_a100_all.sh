...
run_and_log "$LOG_DIR/ablations/throughput_auto_triton_on.log" \
  python benchmarks/bench_throughput.py \
    --model "$MODEL" --dtype "$DTYPE" \
    --batch-size 32 --num-prompts "$NUM_PROMPTS" \
    --prompt-len "$PROMPT_LEN" --max-tokens "$MAX_TOKENS" \
    --block-size "$BLOCK_SIZE" --num-gpu-blocks "$NUM_GPU_BLOCKS" \
    --prefill-backend auto

run_and_log "$LOG_DIR/ablations/throughput_flash_triton_off.log" \
  python benchmarks/bench_throughput.py \
    --model "$MODEL" --dtype "$DTYPE" \
    --batch-size 32 --num-prompts "$NUM_PROMPTS" \
    --prompt-len "$PROMPT_LEN" --max-tokens "$MAX_TOKENS" \
    --block-size "$BLOCK_SIZE" --num-gpu-blocks "$NUM_GPU_BLOCKS" \
    --prefill-backend flash --no-triton --skip-hf
